...
- [ ] Выполнен кодовый аудит `icon-only` кнопок и изображений по всему `app/`.
- [ ] Добавлены первые изменения в UI-компоненты (17.1, 17.4, 17.7).
- [ ] Подключён `@axe-core/playwright` и создан `e2e/a11y.spec.ts`.
...
**Изменённые файлы:**
- `app/components/qr/QrPreview.vue` — `<img :alt="t('qr.preview.alt', { title })">`.
