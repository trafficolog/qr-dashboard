...
| 24.20 | Settings | ⚠️ | PR5 | `follow-up commits (settings shell + general/profile/team primevue migration)` | Начата migration `/settings`: shell navigation, `/settings/general`, `/settings/profile` и `/settings/team` переведены на PrimeVue InputText/Select/Icon/Tag/Dialog/Message patterns; остальные settings subpages ещё в работе. |
