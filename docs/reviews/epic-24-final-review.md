...
| 24.20 | Settings | ⚠️ | PR5 | `follow-up commits (settings shell + general/profile primevue migration)` | Начата migration `/settings`: shell navigation, `/settings/general` и `/settings/profile` переведены на PrimeVue InputText/Select/Icon/Tag patterns; остальные settings subpages ещё в работе. |
