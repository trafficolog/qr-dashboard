...
| 24.35 | Unit + E2E фиксы | ⚠️ | PR6 | `follow-up commit (test sweep: unit pass, e2e regression baseline)` | `pnpm test:unit` проходит (33/33), `pnpm test:e2e` падает массово (regression wave после UI migration; требуется отдельный sweep сценариев/селекторов и стабилизация playwright smoke). |
