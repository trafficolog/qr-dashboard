...
| 24.35 | Unit + E2E фиксы | ⚠️ | PR6 | `follow-up commits (playwright webServer env/host hardening + harness cleanup)` | `pnpm test:unit` проходит (33/33). Для e2e дополнительно стабилизирован `webServer` (host/env/timeouts), но в текущем окружении `playwright` по-прежнему упирается в startup timeout; финальный зелёный прогон остаётся открытым. |
| 24.36 | A11y sweep | ⚠️ | PR7 | `follow-up commit (a11y sweep started)` | Запущен этап A11y: актуализирован baseline и подготовлена точка входа (`e2e/a11y.spec.ts`) для следующего полного прогона. |
...
- **Частично (in progress):** 24.35, 24.36
- **Не начато:** 24.37+
