...
## Стандарт именования компонентов

Принят единый стандарт для Vue SFC: **PascalCase** (например, `UnsavedChangesDialog.vue`).
Авто-импорт Nuxt для `app/components/shared/UnsavedChangesDialog.vue` остаётся стабильным и резолвит компонент как `SharedUnsavedChangesDialog`, поэтому шаблоны страниц менять не требуется.
