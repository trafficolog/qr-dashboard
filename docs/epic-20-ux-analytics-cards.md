...
**Критерии приёмки:**
- [x] Карта РФ отображается с подсветкой регионов
- [x] Tooltip при hover показывает город и количество сканирований
- [x] Таблица Country/City с сортировкой работает
- [x] Переключатель «Россия / Мир» корректно меняет визуализацию
- [x] При отсутствии гео-данных — EmptyState с текстом «Нет данных о географии»
- [x] DateRangePicker влияет на гео-данные
...
**Критерии приёмки:**
- [x] Три donut-чарта отображают Device, OS, Browser
- [x] Легенда с процентами рядом с каждым чартом
- [x] При пустых данных — «Нет данных» вместо пустого donut
- [x] Адаптивность: 3 → 2 → 1 колонка на разных breakpoints
- [x] DateRangePicker влияет на данные
...
**Критерии приёмки:**
- [x] Bar chart по часам 0–23 отображается корректно
- [x] Horizontal bar по дням Пн–Вс (локализованные названия)
- [x] Пиковые значения визуально выделены
- [x] Skeleton при загрузке
...
**Критерии приёмки:**
- [x] Toggle «Сравнить» отображается рядом с DateRangePicker
- [x] При включении — dashed-линии предыдущего периода появляются на графике
- [x] Tooltip корректно показывает current vs previous
- [x] При выключении — линии исчезают без перезагрузки данных
...
**Критерии приёмки:**
- [x] Grid-карточка показывает URL (truncated), дату, сканирования
- [x] Кнопки Download и Copy URL видны на карточке без раскрытия dropdown
- [x] Copy URL → clipboard + toast
- [x] Hover на миниатюру QR в таблице → увеличенный preview в popover
- [x] Inline icon-buttons в таблице для Download и Copy URL
- [x] Мобильная адаптивность: на <768px кнопки stack вертикально
...
**Критерии приёмки:**
- [x] 7 SVG-иллюстраций создано и оптимизировано
- [x] EmptyState.vue поддерживает prop `:illustration`
- [x] Иллюстрации корректно отображаются в light и dark theme
- [x] Размер каждого SVG < 5 KB (SVGO)
- [x] Страницы `not-found.vue` и `expired.vue` используют иллюстрации вместо plain text
...
**Критерии приёмки:**
- [x] При первом входе нового пользователя — overlay с 3 шагами
- [x] Spotlight выделяет целевой элемент
- [x] «Пропустить» закрывает overlay и помечает `completed`
- [x] При повторном входе overlay не показывается
- [x] Локализован на ru/en
...
### Статус реализации
- Блоки **analytics**, **cards**, **empty states**, **onboarding** — выполнены.
- Пункт **20.8 Usage Widget** не вошёл в релизный scope EPIC 20 и перенесён в следующий UX-пакет.
