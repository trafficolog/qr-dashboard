...
- ⚠️ Settings migration in progress: `/settings` shell navigation, `/settings/general`, `/settings/profile`, `/settings/team`, `/settings/domains`, `/settings/destination-domains`, `/settings/integrations` и `/settings/audit` переведены на PrimeVue InputText/Select/Icon/Tag/Dialog/Message/ToggleSwitch patterns
