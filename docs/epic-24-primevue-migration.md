...
- ⚠️ Notifications migration in progress: `/notifications` переведена на PrimeVue Button/Tag/Skeleton patterns, подключена к `/api/notifications` (источник — audit events текущего пользователя), добавлена персистентная read-state модель через cookie `splat-notifications-read`; pending: расширение payload backend-уведомлений (priority/severity/deeplink)
...
- **Dark mode:** через `darkModeSelector: '.app-dark'` + синхронизация c layout-composable `useLayout()`.
...
**Критерии приёмки:**
- [x] Ветка создана
- [x] `docs/reviews/epic-24-baseline.md` заполнен
...
app/assets/css/tokens.css             — SPLAT-токены на уровне :root/.app-dark (мост к PrimeVue)
...
- [ ] Светлая и тёмная схемы синхронизированы с `.app-dark` селектором
...
**Критерии приёмки:**
- [x] Все 12+ примитивов созданы, документированы, покрыты сторибуками в `/docs-ui` (EPIC 18)
- [x] Каждый компонент имеет корректные TS props
- [x] Визуальный вид совпадает с React-макетами
...
**Критерии приёмки:**
- [x] `grep -rn "^import.*@nuxt/ui\|<U[A-Z]" app/` — 0 совпадений
- [x] `@nuxt/ui` удалён из `package.json`
- [x] `pnpm build` проходит
- [x] Приложение рендерится (визуальные регрессии ожидаемы, фиксятся в задачах 24.9+)
...
- [ ] Theme toggle переключает `.app-dark` class через `useLayout()`
...
**Критерии приёмки:**
- [x] 4 KPI карточки с корректными данными и спарклайнами
- [x] Scan chart рендерит реальные 30-дневные данные (ECharts/Chart.js — берём через vue-echarts, уже в зависимостях)
- [x] Segmented control переключает период без перезагрузки
- [x] Top-5 отсортирован по scans desc и ссылается на детальный drawer
- [x] Activity feed показывает 6 последних событий
...
**Критерии приёмки:**
- [x] Drawer открывается по клику на строку `/qr` или по маршруту
- [x] Full-page version `/qr/[id]` имеет тот же UI (header/actions/stats + расширенный аналитический блок)
- [x] Все действия (Download, Copy, Edit, Pause, Share) работают
- [x] Copy URL показывает Toast «Скопировано»
...
Адаптация под PrimeVue patterns (кнопки + card-like overlay shell) с сохранением spotlight-поведения и шагов onboarding.

**Критерии приёмки:**
- [x] Показывается только при первом входе (`useOnboarding` composable — EPIC 20)
- [x] Skip/complete состояния сохраняются
...
**Критерии приёмки:**
- [x] CRUD departments работает (EPIC 19)
- [x] Members management работает
- [x] Access control сохранён (только admin)
...
**Критерии приёмки:**
- [x] Allowed Domains CRUD работает (EPIC 2)
- [x] Destination Domains whitelist работает (EPIC 21 SEC-07)
...
```
app/composables/useLayout.ts     — theme logic + persistence
app/components/app/AppTopbar.vue — icon-button (sun/moon) + menu toggle
app/layouts/default.vue          — initial theme initialize
app/layouts/auth.vue             — initial theme initialize
```

**Логика:**
1. Initial theme берётся из cookie `splat-theme` (`dark`/`light`), иначе fallback на `matchMedia('(prefers-color-scheme: dark)')`.
2. Переключение темы реализовано через `useLayout().toggleDarkMode()` (Sakai-style).
3. User override сохраняется в cookie `splat-theme` на 1 год.
4. Toggle: только light ↔ dark (без configurator/preset переключателей по Amendment A1).
5. `.app-dark` class на `<html>` включает PrimeVue dark scheme.

**Критерии приёмки:**
- [x] Нет FOUC при hydration (инициализация темы в layout `onMounted` + cookie/fallback)
- [x] User preference persist'ится между сессиями
- [x] Переключение темы работает через `.app-dark` и соответствует Amendment A1 (Sakai baseline)
...
| R-09 | Dark mode в PrimeVue не синхронизируется с layout state | Низкая | Low | Используем `.app-dark` selector + `useLayout().toggleDarkMode()` + cookie persistence. |
