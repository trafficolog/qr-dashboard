...
- ⚠️ Unit + E2E фиксы in progress: `pnpm test:unit` = pass (33/33); для e2e добавлен `webServer` bootstrap в Playwright config, `NUXT_ICON_SERVER_BUNDLE=local` для офлайн-режима и исправлен cookie setup в `auth.spec`, но прогон всё ещё блокируется network-зависимостями `unifont/fontless` при старте dev-server
