...
test.describe('QR quick actions visibility flow', () => {
  ...
  test('updates visibility to public and refetches list', async ({ page }) => {
    ...
    await page.route('**/api/departments/my', async (route) => {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: [{ id: 'dep-1', name: 'Маркетинг' }] }) })
    })

    await page.route('**/api/qr/*/visibility', async (route) => {
      visibilityPayload = route.request().postDataJSON() as { visibility?: string }
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: { id: 'qr-1', visibility: 'public', departmentId: null } }) })
    })
    ...
    await page.route('**/api/qr**', async (route) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          data: [{ id: 'qr-1', shortCode: 'quick-actions', title: 'QR Quick Action', destinationUrl: 'https://example.com/quick-actions', status: 'active', totalScans: 12, createdAt: '2026-01-01T00:00:00.000Z', visibility: listFetchCount >= 2 ? 'public' : 'private', departmentId: null }],
  ...
  test('department action is disabled when user has no departments', async ({ page }) => {
    ...
    await page.route('**/api/qr**', async (route) => {
      ...
      await route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify({
          data: [{ id: 'qr-2', shortCode: 'no-deps', title: 'QR No Departments', destinationUrl: 'https://example.com/no-departments', status: 'active', totalScans: 5, createdAt: '2026-01-01T00:00:00.000Z', visibility: 'private', departmentId: null }],
          meta: { total: 1, page: 1, limit: 20, totalPages: 1 },
        }),
      })
    })
