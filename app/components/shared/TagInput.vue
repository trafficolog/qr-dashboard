<template>
  <div>
    ...
    <div
      ...
      class="relative z-10 mt-1 max-h-40 overflow-y-auto rounded-lg border border-[color:var(--surface-border)] bg-[color:var(--surface-card)] shadow-lg shadow-black/5"
    >
      <button
        v-for="tag in filteredSuggestions"
        :key="tag.id"
        type="button"
        class="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-[color:var(--text-color)] hover:bg-[color:var(--surface-hover)]"
